...
| Destination facts during generation | Active | 2026-08-17 | Claude | Three true, trip-specific facts sit still beneath the caption, cross-fading as a set every 9s, plus a line above the disc naming the trip () — the plan form unmounts during generation, so nothing else on screen said which trip was being built. **No model call**:  is nearly all time-to-first-token, so fetched trivia would land after the plan it was meant to cover;  (pure, type-only imports, unit-tested) derives everything from data already fetched and never fabricates. Facts render *outside* the  region so a screen reader can reach them without their being announced every few seconds. Replaced the orbiting version above: 23 infinite animations → 11, four moving 56px backdrop-filters → none, and the reduced-motion branch collapsed into one path because the fallback was the better design |
