...
| Orbiting destination facts during generation | Active | 2026-08-17 | Claude | Two wheels with off-screen centres carry true, trip-specific facts past the loader while the itinerary is written. **No model call** — `runClaude` is nearly all time-to-first-token, so fetched trivia would land after the plan it was meant to cover; `buildDestinationFacts` (pure, type-only imports, unit-tested) derives everything from data already fetched. Never fabricates: a thin bundle yields fewer facts, and under three the wheels don't run at all. Time maps onto angle non-linearly so a card crawls through its readable apex (~28px/s rather than ~75px/s). Mobile drops the right wheel *and* lifts the left one's apex above the disc — measured, a readable ~290px card and the 200px disc cannot share a horizontal band at 375px. Reduced motion swaps the wheels for a static list rather than leaning on the blanket rule, which would snap every card to its off-screen final keyframe and delete the facts outright |
