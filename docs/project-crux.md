...
| 2026-08-22 | Project, Frontend | Zaid | Ran the product question through a 5-advisor council with peer review; the verdict was unanimously "demo, not product", and its one recommended action was to put an itinerary in front of ~5 strangers before building any infrastructure. Recorded in the new `product-readiness.md` tracker with a 7-row gap table and an empty reviewer-findings table, and acted on: added `/trip/[id]/print` (an all-days reviewable document with numbered stops and a fixed review-question sheet) plus the `@media print` block that makes any page in this app printable at all, rewrote the one-word `README.md` with a real pitch and an honest status section, and corrected the stale Sonnet-5 claim in `itinerary-quality.md`. Deploy, hosted DB, auth, an in-app feedback widget and the B2B pitch were all deliberately deferred as downstream of a validation result that does not exist yet |
| 2026-08-23 | Project, LLM | Claude | Priced a whole-itinerary generation off the existing 237 `llm_traces` rows (gap G5 in `product-readiness.md`): **$1.09 and 5.8 min of model time per generation** (median) on `claude-sonnet-4-5`, against $0.235 for a refine turn. Three findings the totals hide — `critique` is 34% of spend and costs nearly as much per call as the generation it checks, making it the biggest cost lever in the app; prompt caching is paid at the 1.25× creation rate on ~34.5k tokens per call and read back on 2 of 15 calls, because a one-shot subprocess per trip means no prompt ever repeats; and failed calls store no envelope at all, so their spend is unknown rather than zero, which makes every figure a floor. Measurement only — no application code changed |
