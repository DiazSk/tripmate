# Itinerary-planner skill: recovering a missing runtime dependency

## Background

`.claude/skills/itinerary-planner/SKILL.md` is read at runtime by `loadSkill("itinerary-planner")`
(`src/lib/skill.ts`) and injected into the prompt by two production routes:

- `src/app/api/trip-edit/route.ts` — the chat/element-edit loop
- `src/app/api/trip-generate/route.ts` — the newer staged-pipeline generator (not yet wired into
  the UI)

The file does not exist in this checkout. It never has: `.gitignore:53` excludes the entire
`.claude/` directory, so this content was never committed anywhere, and `git log --all` for the
path (or for any `SKILL.md` ever added, anywhere in history) returns nothing. This was discovered
when `scripts/perf-bench.mjs`'s chat-edit call failed with `ENOENT ... itinerary-planner/SKILL.md`.

Two sibling skills referenced in `docs/frontend.md`/`docs/project-crux.md` (`dev-analytics-fab`,
`floating-widget-ui`) are in the same state, but neither is loaded at runtime — both are dev-time
reference material for building similar UI, not a `loadSkill()` dependency — so they're out of
scope here.

## What this skill needs to cover

Per `CLAUDE.md`: "the skill holds planning *rules*, `trip-context.md` holds *facts only*, and
`src/lib/generationPrompt.ts` holds the *ask* with no trip data." The **legacy** generation path
(`src/lib/itineraryPrompt.ts`, what `POST /api/itinerary` — and the current UI — actually uses)
bakes its planning rules directly into template strings instead of using `loadSkill`, and has
shipped and been tuned over time (see `docs/llm.md`'s Enhancements table). Recovering this skill is
a mirroring exercise for most of its content: the same rules, expressed once, in the one place both
`loadSkill` consumers can read them, so all generation/edit paths behave consistently.

Two things the skill needs have no legacy equivalent to mirror, because the systems they cover are
newer than the legacy path:

- **Traveler-profile pacing** (`pace_resolved`, `mobility_profile`, `crowd_bias`, `family_rules`) —
  computed deterministically in `src/lib/userAnswers.ts`'s `deriveFlags()` from `UserAnswers`, fed
  to the staged-pipeline path via `tripContext.ts`. The chat/element-edit path's `editContext.ts`
  gives only the *raw* answers (`energy`/`crowds`/`group`/`explorer_style`), never the resolved
  flags — so the skill needs to both apply the flags when given, and derive the same result itself
  from raw answers when not, using the exact thresholds `deriveFlags()` already uses.
- **Opening hours and daylight** — `tripContext.ts`'s POI lines carry `openingHours`/`closedDays`,
  and both `tripContext.ts` and `editContext.ts` carry sunrise/sunset per day. Nothing currently
  instructs the model to use these facts to avoid scheduling a stop when it's closed or after dark.

Both are grounded in code that already exists and ships today (`deriveFlags()`, the two
context-builders) — this is still recovery of intended behavior, not new product design.

Explicitly **out of scope**, and why:
- Critique-pass criteria and rebalance rules stay legacy-only (`buildCritiquePrompt`/
  `buildRebalancePrompt` in `itineraryPrompt.ts`) — neither calls `loadSkill`, so mirroring them
  into the skill would be dead content no consumer reads.
- The `add_stop`/`remove_stop`/`replace_lodging` patch-op field shapes for whole-trip chat edits.
  `editPrompt.ts`'s `OPS_SHAPE` only documents `replace_stop`, leaving the model without field-shape
  guidance for the other three ops Mode A can legally emit — a real gap, but a technical
  output-format detail that belongs in `editPrompt.ts` (which already owns `OPS_SHAPE`), not in the
  skill's planning-rules content. Tracked as a separate follow-up.

## Structure

A single markdown file — `loadSkill()` reads exactly one `SKILL.md`, verbatim, so there's no
mechanism for a secondary reference file. Numbered so `docs/llm.md`'s existing cross-references
(`§3b` = food stops, `§4b`/`§4c` = lodging) land correctly without needing to also edit that doc:

- §1 Preferences and destination-context usage — mirrors legacy (`INTEREST_GUIDANCE`,
  `CONTEXT_USE_INSTRUCTION`)
- §2 Weather-adaptive activity choice — mirrors legacy
- §3 Stops — §3a general fields/sequencing, §3b area-level food-stop rule — mirrors legacy
  (`STOP_FIELD_INSTRUCTION`, `STOP_LINES_INSTRUCTION`, `FOOD_STOP_INSTRUCTION`)
- §4 Lodging — §4a general, §4b type selection, §4c alternative/same-lodging-per-city — mirrors
  legacy (`LODGING_INSTRUCTION` + the same-hotel rule in `buildGeneratePrompt`)
- §5 Budget targeting (85-100%) — mirrors legacy (`BUDGET_INSTRUCTION`), with a note on how it
  applies to a small edit rather than a full re-plan
- §6 Tier style — mirrors legacy (`tierStyle()`/`TIERS`)
- §7 Day narrative/summary — mirrors legacy
- §8 Coordinate realism — mirrors legacy
- §9 Traveler profile: pace, mobility, crowd bias, family — **new**, grounded in
  `userAnswers.ts`'s `deriveFlags()`
- §10 Opening hours and daylight — **new**, grounded in `tripContext.ts`/`editContext.ts` fields
- §11 Output format, scoped to full-generation-from-scratch only — **new**: `trip-generate/route.ts`
  stores the model's raw output as `itinerary_md` with no parsing at all, so nothing else defines
  this format (unlike legacy, which has its own `SHAPE_HINT`/`STOP_SHAPE` JSON constants). Structured
  as markdown mirroring the legacy JSON schema's fields (day → weather/summary → lodging → each
  stop's time/category/duration/cost/lat-lng/why/note), so it stays readable now and is trivial to
  parse into the same `Itinerary` shape if the staged pipeline is ever wired into the UI. Explicitly
  scoped to generation-only in its own text, since the chat/element-edit paths already have their
  own JSON output contract (`OPS_SHAPE` in `editPrompt.ts`) supplied after the skill block — the
  skill does not (and must not) also dictate an output format for edits.

## The skill content

The following is the actual, complete content that will become
`.claude/skills/itinerary-planner/SKILL.md` verbatim (frontmatter included — `loadSkill()` strips it
before injecting the body into the prompt).
...
There is no test framework covering skill content directly. Verification, once this is placed at
`.claude/skills/itinerary-planner/SKILL.md`:

- `loadSkill("itinerary-planner")` succeeds (no `ENOENT`) — confirms the file is at the exact
  expected path and frontmatter is stripped correctly.
- A real chat-edit call through `POST /api/trip-edit` (the path that surfaced this gap via
  `scripts/perf-bench.mjs`) succeeds end-to-end and produces a sensible reply/ops.
- A real staged-generate call through `POST /api/trip-generate` succeeds and the stored
  `itinerary_md` matches the §11 markdown structure.
