...
**The hero hand-off is pinned above `lg`.** On top of the wash, a ScrollTrigger holds the hero section in place for one viewport (`start: "top top"`, `end: "+=100%"`, `scrub: true`, `pin: true`) while a scrubbed timeline pushes the photograph from `scale 1` to `1.12`, drifts the type up `-22%` to `opacity 0`, and takes the wash from 0 to 1. The photograph is held and the frame closes on it; ImageRow then scrolls over an already-dark hero rather than cutting to it. Three things make this safe to have reintroduced JavaScript for:

- **It is gated, not global.** `gsap.matchMedia("(min-width: 1024px) and (prefers-reduced-motion: no-preference)")` — GSAP reverts every property it touched when the query stops matching, so a resize to phone width cannot leave the photo stranded at 1.06. Below `lg`, and for anyone who asked for less motion, the CSS scroll-timeline wash above is the whole effect.
- **A scrub is not a loop.** The Chrome trace that stripped this component's JavaScript was measuring four *infinite* tweens plus a pointer parallax, all running whether or not anything was happening. A scrub does work only while the wheel is actually turning.
- **`[data-hero-pinned]` arbitrates the wash.** Both mechanisms writing `.hero-dusk`'s opacity does not resolve gracefully — a CSS animation outranks an inline style, so GSAP would write it every frame and the keyframe would overwrite it every frame. Hero.tsx sets the attribute for exactly as long as its timeline owns the property, and `[data-hero-pinned] .hero-dusk { animation: none }` stands the CSS one down. Whichever is live, the other is off.
...
**Smooth scrolling is a lerp on `scrollTop`, not ScrollSmoother.** GSAP's ScrollSmoother is the obvious choice and it structurally cannot attach here: it makes `#smooth-wrapper` `position: fixed` and *translates* `#smooth-content` inside it, and the window is always its scroller. This app has no window scroll — `.app-shell` is `h-dvh overflow-hidden` because a full-viewport Cesium canvas sits behind every route, and `.content-overlay` is the real scroller. ScrollSmoother has no `scroller` option to point at it, and handing the scroll back to the window would scroll the globe away with the page. `useSmoothScroll` does the same job inverted: it eases the *actual* `scrollTop` toward a wheel-accumulated target. Two concrete advantages over the plugin it replaces — ScrollTrigger needs no `scrollerProxy()` because the position it reads is genuinely the scroll position, and pinning works normally instead of fighting a translated content layer. Deliberately narrow: wheel only (bars, keyboard and anchors stay native), `(hover: hover) and (pointer: fine)` only (touch already has compositor momentum), off under reduced motion, and it hands the event back at either end so overscroll still belongs to the browser. It also yields the moment it sees a scroll it did not write — and that test is *distance from the last written value*, not distance from the target: the first version compared against the target and killed itself on its own opening frame, because at the start of a 600px glide the remaining distance is 528px by design.

**Its glide is time-based, not frame-based, and its duration is NOT the reference's.** `DECAY_PER_60FPS_FRAME = 0.28` lands 95% of a step in ~150ms, and the tick raises it to the power of `dt / (1000/60)` so that curve is identical on every panel. The first version took a flat 0.12 of the remaining distance *per frame*, which is not one animation but a different one per machine: 391ms to 95% at 60Hz, 175ms on the 134Hz display it was built against, 96ms at 240Hz. Anything easing toward a target across frames belongs to elapsed time; a per-frame constant is a bug that only shows up on hardware you do not own.

**Do not copy the reference's `smooth` value here — it was tried and rejected.** vita-travel.webflow.io runs `ScrollSmoother.create({ smooth: 1.2, effects: true })`, and its content measurably takes ~700ms to catch up to a step. Matching that number exactly made this app feel *laggy*, which is the opposite of the soft feel it was copied to get. The number hides a mechanism difference: ScrollSmoother never touches the wheel — native scroll responds instantly and the plugin lags the *rendered* content behind it, so 700ms of trailing costs nothing in responsiveness. This hook preventDefaults and moves the page itself, so every millisecond of glide is a millisecond of real input latency. **Trailing render reads as softness; delayed input reads as lag.** Same duration, opposite result, and only the first scales. Tune this constant against this mechanism, and keep it short.

One thing the reference measurement did settle: it has **no** `data-speed` or `data-lag` on any element — `effects: true` is enabled and unused — so none of its feel comes from per-element parallax. There is nothing to chase there.

**`scrollHeight - clientHeight` is not a reachable scroll position.** Measured on this landing page: the computed maximum was 1421 while the element could only actually reach 1419. Sub-pixel layout, the zoom level, the scrollbar gutter and per-engine rounding each move that true limit independently, so a glide aimed at a *computed* bottom stayed 2px short forever — `delta` never fell under the settle threshold and rAF span for the rest of the session holding the page against its own edge, which is what the end-of-page stuttering was. The loop therefore stops on an **observed** floor rather than a computed one: after each write it reads the value back, and if the element did not move where it was asked, it is against a real boundary and settles there. That question needs no knowledge of why the limit is where it is, which is what makes it right on every engine and at every zoom rather than on the machine it was written on. Clamping the target to the live range each frame is still there, but as the cheap guard; this is the backstop.

**At a limit, ownership beats deferral.** The same stutter had a second half. Handing the wheel back to the browser whenever the target is already clamped — the obvious shape — releases it *while the glide is still in flight*, so the browser scrolls the element natively and instantly toward the limit, that scroll is correctly detected as foreign and kills the lerp, and the next notch restarts it. Held against either end of the page that alternates between easing and jumping on every notch. So while the hook is still moving the page it keeps the wheel, limit or not; only a *settled* scroller at its limit defers, which is the one case where native overscroll is the right answer. `overscroll-behavior: contain` on `.content-overlay` and `.docked-panel` is the other half — nothing above them scrolls, so a wheel running off their end has nowhere legitimate to chain.

**A vertical trackpad drag is not `deltaX === 0`.** Deferring to the browser on `|deltaX| > |deltaY|` — the obvious way to leave a sideways swipe alone — makes the page read as *stuck*. A trackpad's vertical drag carries constant small horizontal drift, and at the start and end of a gesture both components are near zero, so pairs like `dx 0.7 / dy 0.5` are routine. That test drops every one of them, which swallows the opening events of a gentle two-finger drag: the page refuses to move until it is shoved. The guard needs a dominance ratio *and* a floor (`dx > dy * 2 && dx > 2`), which cannot fire on a drag with drift and still catches a real sideways swipe. This one is worth remembering because **neither a synthetic `WheelEvent` nor a CDP-injected one carries `deltaX` at all** — it is invisible to scripted testing *and* to driver-level automation, and only exists on real hardware.

**This whole path is invisible to scripted testing.** `new WheelEvent()` is untrusted, so no browser performs default scrolling for it, and a synthetic probe of the boundary traces a perfectly smooth exponential while a real wheel stutters. `preventDefault()` *is* still observable on an untrusted event, so the fix is verifiable by asserting on `defaultPrevented` at each position rather than by watching the resulting motion. Frame timing is the wrong instrument here too: it measured a flat 6ms median at the top, middle and bottom while the bug was live, because the stutter was positional, not dropped frames.

Wheel deltas are normalised through `pixelDelta()` before they reach the target, because `deltaMode` is lines on Firefox with a real mouse and pages in a few other cases — treating a `deltaY` of 3 as three pixels makes a full notch move almost nothing.
...
- **Do** call `viewer.stopAutoRotate()` before *any* measurement that depends on where the camera is. The auto-rotate listener runs on `postRender` and will rotate the camera away from a `setView` for the whole settle window. This silently corrupted a tile-detail probe into reporting a 2,054 m geometric error at 900 m altitude before it was caught. Two things changed since: the loop is capped at 60 fps rather than the display's ~160, and under `requestRenderMode` it stops entirely once the idle drift has eased out — so a probe must now also assume the scene may be *asleep*, and call `scene.requestRender()` (or move the camera) to get a frame at all rather than waiting for one that will never come.
