...
- **`Hero`** — the opener: one word, a support line, and "Plan a trip". **Two photographs with the headline between them.** A back layer hangs from the top edge, a front layer stands on the bottom, they overlap in the middle, and "Somewhere." sits at `z-2` — behind the front layer, in front of the back one. So the steppe's horizon cuts across the bottom of the word rather than stopping beneath it. The subline and CTA sit at `z-4`, deliberately above the front layer, so the support copy stays fully legible while the headline is the only thing occluded. This is the reference's own construction, read off its live DOM rather than inferred.
  ...
  **In the portrait composition the CTA goes full width between the gutters, and stays in the centred block.** That is the reference's `full-width-mobile` button variant. Docking it to the section's floor — the obvious reading of "put it at the bottom" — was built and reverted, and the reason is the aspect-ratio decision above it: this section's height follows its *width*, so it is deliberately taller than a phone screen and its floor sits roughly 200px below the fold. The button is at ~477px of an 870px hero today, which is on screen; the bottom of a section that runs past the viewport is *further* from the reader, not closer. Pulling it out of the flow costs a second thing too — the centred block loses ~85px, the headline drops with it, and "Somewhere." sinks below the horizon instead of being cut by it, which is the one effect this whole two-layer construction exists to produce. Reaching both gutters needs the mobile `w-full` on the otherwise shrink-to-fit text wrapper as well, or the button sizes to the subline's `max-w-[22rem]`.
