...
- **Collage variant** (`trips.length > 0`): up to five `HeroTile`s in a bento grid — `grid-cols-2 grid-rows-2` below `sm`, `sm:grid-cols-[1.3fr_1fr_1fr]` with the first tile `row-span-2` above it — separated by a 2px `gap-0.5` over a `--surface-deep-rgb` ground, so the seams between photographs are the system's own material rather than a border colour. The same five DOM tiles serve both templates; on the narrow 2×2 the section's own `overflow-hidden` quietly clips the two that don't fit, rather than a second conditionally-rendered layout that would have to be kept in sync.
- **Tiles follow the Constant-Ground Rule**, the same way `MemoryPostcard` and `ItineraryCard`'s `BlurredPhotoLayer` do: each tile is a permanent solid slate base with `usePlacePhoto(trip.destination, "full")` fading in over it on `.value-in`. A photo miss or a rate-limited lookup dims a tile; it never opens a hole in the hero back to the live globe.
