...
The gate has a material consequence that follows automatically: a surface with no globe behind it has nothing to frost, and a 56px blur of a flat `--canvas` is a blur of nothing while still costing a render surface. So `.glass-itinerary.is-opaque` is the frost-off variant, carried by the four surfaces where `globeWanted` is false — `/trips`' memory cards, the plan step's form card, and the two dead-end cards (`/trip/latest` on an empty database, and `/trip/[id]`'s not-found). `/profile` went further and has no panel at all — see **The Profile Page Has No Panel**. **Adding a glass surface to a route means checking whether that route boots the globe first.** The two exceptions worth knowing: `.glass-nav` keeps its blur everywhere, because content scrolls under it on every route; and `ConfirmDialog`'s panel keeps its blur even on `/trips`, because it sits over its own `::backdrop` — 55% black plus a 4px blur over the live page — and blur over a dimmed page is not blur over nothing.

**The Profile Page Has No Panel.** `/profile` sits directly on `--canvas`: no card, no border, no
shadow, no blur. Its sections are separated by hairlines and space, the way `FeaturedPlans` and
`DestinationMap` already treat their own globe-less sections. Adopted from the reference's contact
page, which is borderless throughout — but the reason it is *correct* here rather than merely
fashionable is the Mounted-Surface Gate: the panel existed to separate a settings form from a live
globe, and there is no globe on this route to separate it from.

The page is a two-column split above `lg` — a memories column of real photography on one side, the
form on the other, which is the reference's own contact-page arrangement. Single column below that:
two columns on a phone give the photograph a strip too narrow to read as a photograph and the
pickers too little room to stay tappable. The memories column is `sticky`, which this page needs
and the reference's short contact form did not — without it the photography scrolls away in the
first screenful. `items-start` on the grid is what makes that possible at all, since a stretched
grid item has no free space to stick within.

<details><summary>The Profile Panel Tint — Eliminated 2026-08-20</summary>

`/profile`'s form sat on `.profile-glass`: `.glass-itinerary`'s material (the one slate, hairline,
panel ambient shadow) with a cobalt-teal gradient laid over it, reusing Blue Hour's
`--scene-cobalt-rgb`/`--scene-teal-rgb` at the alphas `.scene-band` established, and deliberately
*not* retinting `--accent` so pickers and Save stayed amber. It was a tint on the one slate rather
than a new grey, which is what kept it inside the One Slate Rule.

It died in two steps, and the second one is the lesson. First the Mounted-Surface Gate took the
globe off this route, which made the blur a blur of nothing — so the panel went opaque and kept its
tint. That left a tinted, shadowed, hairlined rectangle floating on a flat canvas, which is a card
drawn around content for no reason other than that a card used to be needed there. **When the
reason for a surface is removed, check whether the surface should go too, not just whether it
should be restyled.** The accent-retint reasoning it recorded is still live and still correct — it
is preserved in the `.blue-hour-scene` entry, which is the rule it was arguing against.
</details>
...
**The Unlayered-Beats-Utility Rule.** Every component rule in `globals.css` — `.glass-control`, `.glass-itinerary`, `.memory-card-delete`, `.font-display`, all of them — sits **outside** `@layer`. Cascade layers rank unlayered author CSS above anything a layer emits, *at any specificity*, so a component rule beats a Tailwind utility unconditionally: not by winning a specificity contest, by not entering one. The consequence is that **a Tailwind utility can never override a property a component class already sets**, and it fails silently — the class is in the markup, the rule is in the stylesheet, and nothing happens.

This has now cost real interaction on four separate properties. `box-shadow`: `.glass-control`'s shadow ate `focus-visible:ring-*`, leaving keyboard focus invisible on the mobile sheet handle. `background`: four controls — the 2D/3D toggle, the reset-north compass, that same sheet handle, and `/trip/[id]`'s "Refine with AI", the app's most prominent secondary action — shipped with `hover:bg-white/10` and had **no hover or press feedback at all**, each running a `transition` on a property that never moved. `font-weight`: `.font-display` is 600, so `font-bold` on `/trips`' two headings computed 600. `opacity`: `.memory-card-delete` is `opacity: 0`, so the delete button's `focus-visible:opacity-100` was dead and the keyboard reveal came from an unrelated `:focus-within` rule nobody knew was load-bearing.

So: **a property owned by a component class has its states defined next to it, in CSS.** Not as a utility at the call site. If you find yourself writing `hover:bg-*` on an element carrying a `.glass-*` class, the hover belongs in `globals.css`. When those states are over terrain, they follow the Darken-Never-Lighten Rule — `.glass-control:hover` is the one slate at 0.72 and `:active` at 0.82, not the `white/10` the dead utilities were asking for. The utilities were violating the design system *before* they were dead, which is the tell that the fix is not to make them work.

**The Comment-Is-Not-A-Class Rule.** Tailwind v4's source detection reads every file in the project and does not care that a file is prose. Any token shaped like a utility becomes a real rule in the shipped stylesheet — including one written inside a code comment to explain why it was removed. This repo makes that a standing leak rather than a one-off, because the docs convention requires logging fixed bugs in detail and those entries quote the classes involved. Measured: excluding markdown from scanning dropped the stylesheet from 104,970 to 99,679 bytes — **5.3KB of it was class names lifted out of documentation**, none of it on any element, including a `nth-child(2)` border override deleted from `FeaturedPlans` and a superseded `max-aspect-ratio` z-index variant from the hero. `globals.css` now carries `@source not "../../**/*.md";`. Inside `.tsx` comments, where no such exclusion is possible, name a class in words rather than writing it as a token — and check first whether any element still carries it, because quoting a class that nothing uses is what resurrects it.

**The Inclusive-Both-Ways Rule.** `min-aspect-ratio: 3/5` and `max-aspect-ratio: 3/5` are **both inclusive**, so a pair of them built to be mutually exclusive both match at exactly 3:5 — and there the arrangement degenerates into a per-property emission-order race, silently, at one viewport size. The hero's landscape/portrait split hit this: measured at 375×625 the CTA took its portrait `width` (full-bleed) while keeping landscape `position` and `z-index`, floating 186px above the bottom of a hero it is supposed to be pinned to, with 375×624 and 375×626 each individually correct. Use range syntax — `[@media(aspect-ratio<=3/5)]` / `[@media(aspect-ratio>3/5)]` — which shares no value; Lightning CSS downlevels it to `(max-aspect-ratio:3/5)` and `not all and (max-aspect-ratio:3/5)`, an exclusive pair, within this project's browser floor. Pick one branch to own the boundary and give it *everything*, the `<picture>` `media` attributes included, so the crop and the layout cannot disagree about which branch that one viewport is in.
...
**The Persistent Globe Rule.** `GlobeBackground` is never conditionally rendered, never re-keyed, never moved. A remount destroys the Cesium viewer, loses the camera pose and re-fetches the 3D tiles. Route-level differences are expressed by what floats above it — and *whether* it is behind them at all is **The Mounted-Surface Gate** above, which is a different question from this one.
...
- **`Hero`** — the opener: one word, a support line, and "Plan a trip". **Two photographs with the headline between them.** A back layer hangs from the top edge, a front layer stands on the bottom, they overlap in the middle, and "Somewhere." sits at `z-2` — behind the front layer, in front of the back one. So the steppe's horizon cuts across the bottom of the word rather than stopping beneath it. The subline and CTA sit at `z-4`, deliberately above the front layer, so the support copy stays fully legible while the headline is the only thing occluded. This is the reference's own construction, read off its live DOM rather than inferred.
  ...
  **In the portrait composition the CTA docks to the section's floor, full width between equal gutters.** This is the reference's construction, measured off its live DOM at 402px rather than inferred: `.intro__button.mobile-visible` is `position: absolute; inset: 0` with `align-items: flex-end`, and the anchor inside comes out 351px wide with 26px of gutter left, right and below. Equal inset on three sides, so ours is `inset-x-5` / `bottom-5` — the same idea at this project's own 20px page gutter rather than importing their 25.728px into a page where nothing else uses it. `inset-x-5` and not `inset-x-0`, because an absolutely positioned element's containing block is its ancestor's *padding box* and would otherwise ignore the section's `px-5`.

  Two things had to follow, and both are the interesting part.

  **The section reserves the button's space with a mobile `pb-[5.375rem]`.** Taking the button out of the flow shortens the centred text block, which re-centres ~45px lower — the first attempt dropped "Somewhere." below the horizon instead of being cut by it, losing the one effect the two-layer construction exists to produce. The reference never hits this because its text block is placed by `padding-top` rather than centred, so its button leaving the flow moves nothing. Ours stays centred and reserves instead. The 86px is measured, not derived: the button's own footprint is 73px and leaves the headline at 346px, because the block's real height is 106px rather than the 118px the type metrics suggest. Holding the headline at the 339px it sat at before needs `(870 - pb - 106) / 2 = 339`. Re-measure if the subline's copy or the type step changes.

  **The docked CTA sits at `z-11`, above `.hero-dusk`.** That wash is the scroll-driven fade of this hero into the next beat — `rgb(var(--surface-deep-rgb))` at `z-10`, resting at `opacity: 0`. Because the hero's height follows its *width* it is deliberately taller than a phone screen (~870px against a visible ~700px once Safari's chrome shows), so the floor and the button on it start below the fold, and *reaching* the button means scrolling — which is exactly what raises the wash. Measured: the CTA clears the fold around scrollTop 450, where the wash is at 0.353 and renders a white pill as `rgb(170,181,184)`. The primary action was arriving on screen already greyed out. Above the wash it stays white while the scenery fades behind it, which is the right division — the photograph is leaving, the button is not. Landscape keeps `z-4`: there the button is centred and visible at rest with the wash at zero, so fading with the composition as the hero scrolls away is correct.

  The below-the-fold start is the reference's behaviour too — Vita ships an 858px hero with its button 26px off that floor — and it falls out of the `aspect-ratio` decision both projects made deliberately.
...
- **The bar carries no horizontal padding; the two cells do.** A rule can only span the bar's full height if the box carrying it is that tall, so the bar is `items-stretch` and the cells re-centre their own content. Left as `items-center`, the wordmark cell measured 45px in a 65px bar and the rule stopped short — the identical failure that left the profile split's rule ending halfway down the page.
- **Both rules are gated on the route being user-facing** (`isUserFacing`, see the route table below), not on there being links to divide from. That earlier gate is what made `/profile` render a bare strip while every sibling rendered a grid: it carries no outbound nav links, so there was nothing to divide from and the rule vanished. The trailing cell is what makes the gate unconditional per route — `/profile` has no links and still has a cell. What stays bare is `/backend`, `/backend/pipeline`, `/bench` and any unmatched URL (Next renders its built-in 404 *inside* the root layout, so this nav mounts there too): two internal dashboards and a dead end, none of which should carry a user-facing profile link.
- **The bottom rule replaced the mobile menu's own `border-t`.** The menu is `top-[var(--nav-h)]` and the bar's border occupies the last pixel of that height, so the two abutted exactly and drew one hairline twice.

**Three cells, the reference's own count.** This entry used to say two and defended it: the reference's third is a distinct "Explore" CTA, and walling off `Profile` — a peer link, preceded on `/trips` by `New trip` — would assert a CTA that does not exist. That argument mistook the rule for a CTA frame. It is the mirror of the wordmark's, and without it the bar is a grid drawn down a single side. The trailing cell holds whatever this route's one trailing control is: `Profile` everywhere, the menu toggle on `/` below `sm`, where `MENU_ITEMS` already carries `Profile` into the panel and showing it in the bar too would put one destination in two places. So the cell is never empty and never holds two things.

On `/profile` that link is the current page — `navLink(true)` plus `aria-current="page"`, never colour alone, and `"page"` rather than the `"location"` the section anchors carry, since those mark a position within the document and this marks the document. It stays a `Link` rather than inert text so the bar's last tab stop exists on every route the bar is a grid on. **The breakpoint on it rides a wrapper, and has to:** `navLinkBase` already sets `inline-flex`, and Tailwind emits display utilities alphabetically at equal specificity, so appending `hidden` to that link is a silent no-op — measured in this app's own compiled CSS, `.hidden` at byte 16947 against `.inline-flex` at 17026. Emit one display utility per branch, never two and a guess about order. Known limit, accepted rather than solved: white at 0.12 is reliable over the slate and over dark terrain and fades toward invisible over a bright map or the dashboards' stone-50 ground. That is the tradeoff `.glass-nav`'s tint already takes over unpredictable imagery, and nothing about the bar depends on the rule to stay legible.

**Below `sm` the bar draws one vertical rule, not two.** The wordmark's is `sm:`-gated; the trailing cell's is not. That is the reference's own phone arrangement — there the bar is a wordmark and a toggle, and it rules off only the toggle. Drawing both at 375px put two hairlines either side of a middle cell that is empty on `/`: three divisions in a bar that narrow, two of them fencing nothing. The trailing rule is the one that survives because it is the one with a control against it.
...
It is also the one place per-route utility links live now: the wordmark (`href="/"`), section anchors ("The Journey" → `ImageRow`, "How It Works" → `HowItWorks`, landing only), and the route-aware actions — "My memories" on `/` and `/trip/[id]`, "New trip" on `/trips`, and "Profile" on every user-facing route including `/profile`, where it marks the current page. These used to be four separate bare-canvas links (the closing poster's CTA row, a lone link on the plan step, `/trips`, `TripView.tsx`), each floating in and out with its own page's layout; consolidating them removed the `headerLinkClass` pattern entirely; a page no longer supplies its own top-right link.

**The route table is an explicit list, not a negation.** The bar's cells render on `isUserFacing = isHome || pathname === "/trips" || isTripDetail || isProfile`, rather than on a `!isInternal` catch-all that reads cleaner and is shorter. The catch-all silently swept in `/backend` and `/backend/pipeline` — internal dashboards that render this same `Navbar` over their own stone-50 pages and have no business carrying a user-facing profile link. It has a second victim now that the trailing cell exists: the built-in 404, which a negation would hand a Profile link on a dead end. A nav predicate that describes *where a link belongs* survives a new route being added; one that describes *where it doesn't* quietly adopts every future route.
...
**Mobile menu: a full-screen surface, not a dropdown.** Below `sm`, the section anchors, "My memories" and "Profile" move behind a hamburger toggle rather than being hidden outright. It began as a dropdown as tall as its own content and is now `fixed inset-x-0 top-[var(--nav-h)] bottom-0` — everything below the bar, which is the reference's own arrangement. Five things distinguish it from a naive show/hide toggle:
...
- **The panel stays mounted, and what animates changed with its geometry.** Instead of conditionally rendering it (which pops in and simply vanishes on close, animating only one direction), the panel is always in the DOM. It used to transition its wrapper's `grid-template-rows` between `0fr` and `1fr` — the CSS grid auto-height trick — which is retired here rather than broken: once the panel *is* the viewport there is no height to animate between. Opacity plus a 6px settle carries both directions instead, on the same `cubic-bezier(0.16, 1, 0.3, 1)`, and rows still fade in on a 60ms-per-row stagger with the CTA arriving after the last of them.
- **The bar and the panel are one field while it is open.** `.glass-nav` is the one slate at 0.15 with a 12px blur, which is right over a live globe and wrong butting against a solid panel — the header row read as a visibly lighter strip across the top of one surface. `.glass-nav.is-menu-open` takes the bar to the same opaque slate and drops its blur for the duration, so the only line in the composition is the panel's own `border-t`. **The panel itself is `.glass-nav-menu` at alpha 1 — the top of the more-alpha-for-more-content step, not a departure from the material.** Raising alpha within the one slate is the move the system allows; reaching for `--canvas` would have been leaving it. Two things earned the last 15% over the old 0.85: a full-screen panel has the whole page behind it rather than a sliver of nav, and on a phone the footer photograph was legibly ghosting through the menu rows. The blur goes with it, by the same reasoning `.glass-itinerary.is-opaque` already uses — nothing shows through an opaque surface, so a 20px blur is a compositing layer rendering a blur of nothing, and here it fired during a full-viewport fade.
- **`inert` when closed, replacing per-row `tabIndex` bookkeeping.** One attribute takes the whole panel out of the tab order, out of the accessibility tree, and out of hit-testing; the old `tabIndex={menuOpen ? 0 : -1}` on every link was correct but was per-row state a new row could forget, and said nothing about the panel as a whole. `pointer-events-none` rides along rather than trusting `inert` alone to stop a tap — an invisible full-viewport surface swallowing taps meant for the page is a failure this project has already documented. Verified by hit-testing the closed panel's centre and confirming it reaches the hero photograph beneath.
- **Escape replaced close-on-outside-tap, and returns focus to the toggle.** The old `pointerdown` listener made sense for a dropdown with the page still beside it. Full-screen there *is* no outside — every tap that is not the X or a row lands on the panel — so the listener could only ever have fired on the bar it excluded. What is deliberately **not** here is a focus trap: tabbing past the last row reaches page content behind the panel. That is a real if small hole, recorded rather than closed, in the same spirit as the section-anchor gap above.
**`SiteFooter` is on every scrolling, user-facing surface** — the landing, `/trips` and `/profile` — not just the landing that first needed it. It takes no props and knows nothing about a scene, so its old home in `blue-hour/` only ever described who imported it; it lives in `components/` now. Each host cancels its own `<main>` padding around it (`-mx-5 -mb-*`, the technique `ScrollStory` and `/trips`' hero already use), because the footer is a full-bleed band with its own gutters and its own `pb-8` and has to reach three edges rather than sit inside the column above it. On `/trips` that cancel is conditional on there being trips, mirroring the hero's: the empty state's `<main>` carries no padding to cancel.

Its two links stay plain on the route they point at, rather than picking up the `aria-current="page"` treatment the navbar's Profile link carries. A footer is a list of destinations, not a position indicator — and marking one would mean making this a client component for `usePathname()`, which would also drag its `new Date().getFullYear()` into hydration, where today it is a server render with nothing to mismatch.

- **Type is the footer's treatment with the menu's own clamp.** Same 2.81rem ceiling, weight, `-0.085em`, and `+` at white/45 — that scale is already this project's answer to "a short list of destinations set as large as the surface allows", and the menu and the footer are the two places the question comes up. The floor and slope are not copied, though, and copying them was the first mistake: the footer's `clamp(1.75rem, 4vw, 2.81rem)` resolves to its floor on a 402px phone and sets 28px, which is right in a footer that also carries a wordmark and small print, and wrong on a panel that *is* the viewport, where it read as a dropdown that had merely grown. `clamp(2rem, 10vw, 2.81rem)` reaches the ceiling by 450px and gives 40px at 402px — measured, a 44px line box, which is also what puts these rows on the touch-target floor rather than the 31px the footer's curve was giving them. The lucide icons that used to sit beside the labels are gone: an 18px glyph next to 40px type reads as a bullet, and the reference sets these as bare words.
- **It ends on a pinned CTA.** A full-width white pill at the panel's floor via `mt-auto`, reusing the hero CTA's classes minus its `shadow-lg shadow-black/30` — that shadow lifts a white pill off a photograph, and on a flat slate field it would be decoration with nothing to separate from. It closes the menu and scrolls the container to top rather than opening the wizard directly, because `onPlan` is local state in `HomeView` and this component is a sibling of the content overlay with nothing to call; reaching it needs the same lifting the section-anchor gap describes.
...
- **Don't** put a `hover:`, `active:`, `focus-visible:` or any other state utility on an element carrying a `.glass-*` class, or on any property a `globals.css` component rule already sets. It will not apply — see The Unlayered-Beats-Utility Rule. Define the state beside the base rule in CSS instead.
- **Don't** write a class name as a token inside a comment. Tailwind's scanner reads comments and will emit it as a real rule; name it in words. See The Comment-Is-Not-A-Class Rule.
- **Don't** pair a base utility with a variant override for the same property and trust the order. Tailwind v4 sorts by *utility*, not by variant, so the override can be emitted first and lose at equal specificity no matter which media query wraps it. **Emit one declaration per property per range** — disjoint ranges (`max-xl:` and `xl:`), never a rule plus a guess. This has bitten five times now: the hero's layer widths, the navbar's Profile-link display utilities, `FeaturedPlans`' card hairlines, `TripsView`'s heading weight, and the hero's aspect-ratio branches. `ImageRow.tsx:100` still carries the pattern and currently works only because `border-t` happens to sort before `border-t-0` — correct today, one Tailwind release from not being.
