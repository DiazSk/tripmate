...
- The globe is live when it is being used and a still picture when it is not. It drifts in exactly one place — the landing story's reveal, the one section that shows it — and is motionless everywhere else; no other ambient motion competes with it. See **The Persistent Globe Rule**.
...
**The Persistent Globe Rule.** `GlobeBackground` is never conditionally rendered, never re-keyed, never moved, and route-level differences are expressed by what floats above it. What *is* conditional is the Cesium viewer inside it, which has three states:

- **`off`** — no viewer at all. A static poster (`GlobePoster`) stands in: a CSS earth-limb still, or the destination's photograph once one is known. This is where every route starts, and where the ones that only ever occlude the backdrop stay.
- **`static`** — the real globe, parked at the hero pose with camera input disabled. Under `requestRenderMode` it draws the frames it takes to stream in and then stops dead. This is the landing page: a photorealistic Earth behind the hero copy that costs nothing to keep on screen. It is also the only mode that drifts — the reveal section asks for the idle rotation while it is on screen (`setGlobeSpinning`), which is bought motion rather than ambient motion: a rotating camera renders every frame, so it runs for the seconds that section is visible and nowhere else.
- **`live`** — the same viewer with input enabled, flying and drawing on demand. Reached the moment the globe stops being scenery: "Plan a trip" is pressed, a destination is chosen, an itinerary stop or marker is clicked, Play tour starts, or the poster's own "Explore in 3D" is used.

`static` and `live` differ by exactly one flag (`enableInputs`) and must never rebuild the viewer between them — a rebuild is seconds of re-fetched tiles. `resetToHome` returns `live` to `static` and flies the camera home rather than tearing anything down. All three states are framed to cut the screen in the same place, so moving between them moves nothing in the composition.

This is a deliberate narrowing of what this rule used to say, and the reason is cost, not looks. Keeping a viewer alive *and rendering* on every route meant a WebGL context, a Google tile stream and a 60fps render loop running behind screens whose backdrop is, visually, a photograph. What actually had to go was the **rendering**, not the globe: once a parked viewer under `requestRenderMode` costs nothing per frame, the landing page can have a real Earth behind its hero copy again, and only the surfaces that never show the backdrop at all stay on the poster. **Nothing that only draws should reach `live`** — `showDayRoute` and `showHighways` queue their geometry rather than promote the mode, because a day route is drawn for an overview and an overview is a picture.
