...
- **The vertical rule is gated on there being links to divide from.** `/profile` deliberately carries no outbound nav links (the route table below; `BackButton` is how you leave) and neither do the `/backend` dashboards, so on those the bar is the wordmark alone and the rule is absent. A rule with nothing to its right divides nothing and reads as an unfinished edge.
...
Deliberately **two cells, not the reference's three.** Its third is a distinct "Explore" CTA; this bar's trailing item is `Profile`, a peer link, and on `/trips` it is preceded by `New trip` — walling either off would assert a CTA that does not exist. Known limit, accepted rather than solved: white at 0.12 is reliable over the slate and over dark terrain and fades toward invisible over a bright map or the dashboards' stone-50 ground. That is the tradeoff `.glass-nav`'s tint already takes over unpredictable imagery, and nothing about the bar depends on the rule to stay legible.
