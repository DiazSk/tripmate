...
While an itinerary generates, three true facts about *this* trip sit still beneath the caption pill — upright, centred, cross-fading as a set every 9s, re-readable at any moment. Above the disc, one line names what is being made: `Kyoto · Sep 19 – 22, 2026 · Mid-range`. The plan form unmounts during generation, so without that line the screen never once states which trip it is working on, which is the thing a waiting traveller most wants confirmed.
...
- **Nothing moves in space.** Opacity only, on the group rather than per line, so a set change reads as one page turn instead of three lines flickering out of step.
