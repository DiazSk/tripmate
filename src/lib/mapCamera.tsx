 ...
 *
 * Resting the pointer on an itinerary row flies to half the distance between where the camera
 * already was and the stop being pointed at — literally "twice as close", which is what makes it
 * read as leaning in rather than going somewhere. A fixed altitude was tried first and was wrong:
 * from a day framed at 4km it is a 2.7x dive, from a tight day already at 2km it barely moves.
 * The same number cannot be a gentle lean for both, because "how much zoom is a lot" is a
 * question about where you started.
 *
 ...
 *  window is the cost. A third of the click's 2.5s, which also keeps it a glance, not a journey. */
...
export function MapCameraProvider({ children }: { children: ReactNode }) {
  ...
  const showDayRoute = useCallback((stops: RouteStop[]) => {
    // A new day reframes the camera, so any peek's saved pose belongs to a view that is about to
    // stop existing.
    cancelPeek();
   ...
   * tiles — the pull-out frames would show an intermediate altitude nobody asked about.
   */
  useEffect(() => {
    const viewer = viewerRef.current;
